...
1. `left_panel_startup_catalog` — The left panel shows the selected workspace's sessions with deterministic order and active markers.
2. `left_panel_long_folder_scroll` — Large session lists scroll predictably with stable focus and row boundaries.
...
8. `left_panel_external_session_inclusion` — External sessions added during refresh appear in the left panel after `order_with_user_preferences` recomputes the row order.
9. `left_panel_no_legacy_grouped_mode` — No legacy grouped-folder `+ more` rows or folder overflow are rendered; every workspace session is shown.
...
7. `refresh_adds_external_session` — Refreshes that surface an unknown external session ID append it via `order_with_user_preferences` and persist the new order.
8. `refresh_preserves_user_order` — Refreshes that do not introduce new sessions do not rewrite `~/.config/ratsus/session-order.json`.
