...
pub fn parse_chat_sessions(output: &str, working_dir: &Path) -> Vec<ChatSession> {
    output
        .lines()
...
fn parse_nexus_session_line(line: &str, working_dir: &Path) -> Option<ChatSession> {
...
mod tests {
    ...
    fn ignores_table_header_and_borders() {
        ...
        assert!(parse_chat_sessions(output, std::path::Path::new("/tmp/nexus-demo")).is_empty());
    }
}
