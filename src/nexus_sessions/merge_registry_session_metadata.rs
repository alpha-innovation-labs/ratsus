...
pub fn merge_registry_session_metadata(
    existing: &NexusSession,
    registry: NexusSession,
) -> NexusSession {
...
mod tests {
    ...
    fn keeps_existing_title_when_registry_title_is_placeholder() {
        let existing = NexusSession::new("old", "Real title", "id", "/tmp/project");
        let registry = NexusSession::new("new", "New Session", "id", "/tmp/project");
    ...
    fn applies_registry_running_status() {
        let existing = NexusSession::new("old", "Real title", "id", "/tmp/project");
        let registry =
            NexusSession::new("new", "New Session", "id", "/tmp/project").with_running(true);
