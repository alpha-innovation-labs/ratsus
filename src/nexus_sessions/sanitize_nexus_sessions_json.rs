...
pub fn sanitize_nexus_sessions_json(output: &str) -> String {
    sanitize_json_control_characters(output)
}

#[cfg(test)]
mod tests {
    use super::sanitize_nexus_sessions_json;
    ...
    fn escapes_raw_newline_inside_string() {
        ...
        assert_eq!(
            sanitize_nexus_sessions_json(output),
            "[{\"title\":\"hello\\u000Aworld\"}]"
        );
    }
    ...
    fn preserves_whitespace_outside_strings() {
        ...
        assert_eq!(sanitize_nexus_sessions_json(output), output);
    }
}
