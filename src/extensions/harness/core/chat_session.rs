...
/// Backend-neutral kind for an entry shown in the session list.
...
/// Session metadata returned by a chat harness.
...
impl ChatSession {
    ...
    /// Returns the same session metadata with the requested session kind.
    pub fn with_kind(mut self, kind: ChatSessionKind) -> Self {
        self.kind = kind;
        self
    }
