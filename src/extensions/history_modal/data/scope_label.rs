...
pub fn history_modal_scope_label(app: &AppState) -> String {
    format!(
        "{} Workspace  {} All",
...
fn workspace_scope_marker(app: &AppState) -> &'static str {
    if app.history_modal.folder_filter.is_some() {
...
fn all_scope_marker(app: &AppState) -> &'static str {
    if app.history_modal.folder_filter.is_none() {
        return ACTIVE_SCOPE_MARKER;
    }
    INACTIVE_SCOPE_MARKER
}
...
mod tests {
    ...
    use super::history_modal_scope_label;
    ...
    fn marks_workspace_scope_active() -> anyhow::Result<()> {
        ...
        app.history_modal.folder_filter = Some(PathBuf::from("/workspace/alpha"));

        assert_eq!(history_modal_scope_label(&app), "● Workspace  ○ All");
        Ok(())
    }
